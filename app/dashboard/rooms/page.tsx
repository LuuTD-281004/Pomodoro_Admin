...
import { getAllGroupRooms, getAllPersonalRooms } from "@/axios/rooms";
...
export default function RoomsPage() {
  ...
  const fetchGroupRooms = async () => {
    setLoading(true);
    try {
      const response = await getAllGroupRooms(
        groupPage,
        limit,
        groupSortBy,
        groupOrder
  ...
  useEffect(() => {
    fetchGroupRooms();
  }, [groupPage, limit, groupSortBy, groupOrder]);

  const fetchPersonalRooms = async () => {
    setLoading(true);
    try {
      const response = await getAllPersonalRooms(
        personalPage,
        limit,
        personalSortBy,
        personalOrder
      ...
      console.error("Failed to fetch personal", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchPersonalRooms();
  }, [personalPage, limit, personalSortBy, personalOrder]);

  const filteredPersonalRooms =
    personalRooms &&
    personalRooms.filter &&
    personalRooms.filter(
      (room) =>
  ...
  const getRoomStatusBadge = (status: number) => {
    const statusMap = {
      0: {
        label: "No Status",
        class: "bg-muted/20 text-muted-foreground border-muted",
      },
      1: {
        label: "Active",
        class: "bg-chart-1/20 text-muted-foreground border-muted",
      },
      2: {
        label: "Rest",
        class: "bg-chart-3/20 text-chart-3 border-chart-3/30",
      },
      3: {
        label: "Completed",
        class: "bg-chart-1/20 text-chart-1 border-chart-1/30",
      },
      4: {
        label: "Long Rest",
        class: "bg-chart-3/20 text-chart-3 border-chart-3/30",
      ...
      <Card className="bg-card border-border">
        <CardHeader>
          ...
          <CardDescription className="text-muted-foreground">
            Manage personal and group focus rooms
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Tabs defaultValue="personal" className="w-full">
            ...
            <TabsContent value="personal" className="space-y-4">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-border hover:bg-muted/50">
                        ...
                        >
                          Room ID{" "}
                          {renderSortIcon("id", personalSortBy, personalOrder)}
                        </TableHead>
                        <TableHead
                          className="text-muted-foreground cursor-pointer hover:text-card-foreground"
              ...
              <div className="flex items-center justify-between px-2">
                <div className="text-sm text-muted-foreground">
                  Showing {(personalPage - 1) * limit + 1} to{" "}
                  {Math.min(personalPage * limit, personalTotal)} of{" "}
                  {personalTotal} results
                </div>
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex items-center gap-1">
                    <span>
                      Page {personalPage}{" "}
                      {personalTotal
                        ? `of ${Math.ceil(personalTotal / limit)}`
                        : ""}
                    </span>
                  </div>
                  <Button
                    ...
                    disabled={
                      personalTotal > 0 &&
                      personalPage >= Math.ceil(personalTotal / limit)
                    }
                    className="border-border"
                  >
                    Next
            ...
            <TabsContent value="group" className="space-y-4">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-border hover:bg-muted/50">
                        ...
                        >
                          Room Name{" "}
                          {renderSortIcon("roomName", groupSortBy, groupOrder)}
                        </TableHead>
                        <TableHead
                          className="text-muted-foreground cursor-pointer hover:text-card-foreground"
                        ...
                        >
                          Code{" "}
                          {renderSortIcon("roomCode", groupSortBy, groupOrder)}
                        </TableHead>
                        <TableHead
                          className="text-muted-foreground cursor-pointer hover:text-card-foreground"
                        ...
                        >
                          Type{" "}
                          {renderSortIcon("roomType", groupSortBy, groupOrder)}
                        </TableHead>
                        <TableHead
                          className="text-muted-foreground cursor-pointer hover:text-card-foreground"
                        ...
                        >
                          Focus Time{" "}
                          {renderSortIcon("focusTime", groupSortBy, groupOrder)}
                        </TableHead>
                        <TableHead
                          className="text-muted-foreground cursor-pointer hover:text-card-foreground"
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell>
                              <Badge
                                variant="outline"
                                className="border-border text-card-foreground font-mono"
                              >
                                {room.roomCode}
                              </Badge>
                            </TableCell>
              ...
              <div className="flex items-center justify-between px-2">
                <div className="text-sm text-muted-foreground">
                  Showing {(groupPage - 1) * limit + 1} to{" "}
                  {Math.min(groupPage * limit, groupTotal)} of {groupTotal}{" "}
                  results
                </div>
                <div className="flex items-center space-x-2">
                  <Button
                  ...
                  <div className="flex items-center gap-1">
                    <span>
                      Page {groupPage}{" "}
                      {groupTotal ? `of ${Math.ceil(groupTotal / limit)}` : ""}
                    </span>
                  </div>
                  <Button
                    variant="outline"
                    ...
                    disabled={
                      groupTotal > 0 &&
                      groupPage >= Math.ceil(groupTotal / limit)
                    }
                    className="border-border"
                  >
                    Next
