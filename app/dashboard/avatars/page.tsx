    ...
    try {
      ...
      const avatarFiles =
        response.files.filter(
          (file: CloudinaryFile) => file.asset_folder === "uploads/avatar"
        ) || [];
      setCloudinaryFiles(avatarFiles);
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-card border-border">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3">
          {cloudinaryFiles.slice(0, 16).map((file) => (
            <div
              key={file.asset_id}
        ...
        {cloudinaryFiles.length > 16 && (
          <p className="text-sm text-muted-foreground mt-4 text-center">
            And {cloudinaryFiles.length - 16} more files...
          </p>
        )}
      </Card>
    </div>
